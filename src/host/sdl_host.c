...
static int host_init(struct host *host) {
  ...
  host->win = SDL_CreateWindow("redcast", SDL_WINDOWPOS_UNDEFINED,
...
int main(int argc, char **argv) {
  ...
  GetThisPath(userdir, MAX_PATH);
  CHECK(userdir);

  char appdir[PATH_MAX];
  snprintf(appdir, sizeof(appdir), "%s" PATH_SEPARATOR "", userdir);
  fs_set_appdir(appdir);
#endif

  /* load base options from config */
  char config[PATH_MAX] = {0};
  snprintf(config, sizeof(config), "%s" PATH_SEPARATOR "emu.cfg", appdir);
  options_read(config);
